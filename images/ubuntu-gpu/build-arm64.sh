...
hubhost_yk=docker.io/crofi

base_image=nvidia/cuda:11.8.0-cudnn8-devel-ubuntu20.04

docker buildx build --platform linux/arm64 -t $hubhost_yk/ubuntu-gpu:cuda11.8.0-cudnn8-$VERSION-$TARGETARCH --push --build-arg FROM_IMAGES=$base_image -f cuda/Dockerfile .
docker buildx build --platform linux/arm64 -t $hubhost_yk/ubuntu-gpu:cuda11.8.0-cudnn8-python3.9-$VERSION-$TARGETARCH --push --build-arg FROM_IMAGES=$hubhost_yk/ubuntu-gpu:cuda11.8.0-cudnn8-$VERSION-$TARGETARCH --build-arg PYTHON_VERSION=python3.9 -f cuda/python/Dockerfile .

# docker buildx build --platform linux/arm64  -t $hubhost/ubuntu-gpu:cuda11.8.0-cudnn8-$TARGETARCH --build-arg FROM_IMAGES=$base_image -f cuda/Dockerfile .
